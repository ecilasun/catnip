...
# Sprites
Neko supports 8x8 hardware sprites, controlled by a sprite table in SRAM. To select the table and its length, a single instruction can be used as follows:
```c
...
```
The sprites that are being pointed at have the following format, with each pixel of the sprite made of 8bit 3:3:2 RGBA colors:
```c
{[64 bytes of 3:3:2 packed RGBA pixels]}*N
```

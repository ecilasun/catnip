...
std::string g_numeralsAlsoHex = "0123456789x";
...
std::string g_typenames = "register uint int ushort short bool void char";
...
void ASTDumpTokens(TTokenList &root, STokenParserContext &_ctx)
{
    ...
    for (auto &t : root)
    {
        ...
        if (t.m_Class == ETC_Keyword || t.m_Class == ETC_FunctionCall)
    ...
    // ) Scan for variable usages
    {
        ...
        while (beg != g_TokenList.end())
        {
            if (beg->m_Class == ETC_Unknown)
            {
                ...
                while (vbeg != beg)
                {
                    if (vbeg->m_Class == ETC_VariableDeclaration && vbeg->m_Value == beg->m_Value) // or function
                    {

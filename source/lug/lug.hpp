		...
		while (!done) {
			...
			switch (auto [op, imm, off, str] = instruction::decode(prog.instructions, pc); op) {
				...
				case opcode::accept_final: {
					//std::cout << "opcode::accept_final\n";
					accept(sr, mr, rc, pc);
					result = done = true;
				} break;
				case opcode::predicate: {
					//std::cout << "opcode::accept_predicate\n";
					registers_ = {sr, (std::max)(mr, sr), rc, pc, 0};
				...
				default:
				{
					//std::cout << "opcode::default\n";
					registers_ = {sr, (std::max)(mr, sr), rc, pc, 0}; throw bad_opcode{};
				}
			}
		}
		return result;

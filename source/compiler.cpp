...
class CSimpleCompiler
{
	...
	CSimpleCompiler()
	{
		...
		CVar					= "var"_sx > capture(id_)[CIdentifierLHS]	 					< [this] { NewVariable(*id_); printf("ALLOC %s, 4\n", id_->c_str()); };
		CVarStatement			= CVar > ";" 													< [this] { };

		CAssignmentStatement	= capture(id_)[CIdentifierLHS] > "="_sx > CExpressionStatement	< [this] { uint32_t V = stack_.top(); stack_.pop(); AssignVariable(*id_, V); printf("SET [%s]\n", id_->c_str(), V); };

		CExpressionStatement	= CExpression > ";"_sx											< [this] { }
								| ";"_sx														< [this] { };
		CExpression				= CTerm > "+"_sx > CExpression									< [this] { uint32_t A = stack_.top(); stack_.pop(); uint32_t B = stack_.top(); stack_.pop(); printf("ADD\n", A, B); stack_.push(A+B); }
								| CTerm > "-"_sx > CExpression									< [this] { uint32_t A = stack_.top(); stack_.pop(); uint32_t B = stack_.top(); stack_.pop(); printf("SUB\n", A, B); stack_.push(A-B); }
								| CTerm															< [this] { };
		CTerm					= CFactor > "*"_sx > CTerm										< [this] { uint32_t A = stack_.top(); stack_.pop(); uint32_t B = stack_.top(); stack_.pop(); printf("MUL\n", A, B); stack_.push(A*B); }
								| CFactor > "/"_sx > CTerm										< [this] { uint32_t A = stack_.top(); stack_.pop(); uint32_t B = stack_.top(); stack_.pop(); printf("DIV\n", A, B); stack_.push(A/B); }
								| CFactor														< [this] { };
		CFactor					= "("_sx > CExpression > ")"_sx									< [this] { }
								| CConstant
								| CIdentifierRHS;

		CTranslationUnit		= CAssignmentStatement
